...
// Sub Total = Qty x Price/Unit, SST Total = Sub Total x SST, Grand Total = SST Total + Sub Total
const TEMPLATE_EXAMPLE = [
  "OPEX", "Software", "Business Productivity - IT", "2026", "16ISI", "Microsoft Malaysia",
  "2026-01-15", "PHN/IT/2026/001", "PO-88231", "Annual",
  "QT-5521", "INV-99231", "DO-4471", "Microsoft 365 E3 Licensing", "Annual renewal of Microsoft 365 E3 licenses",
  "M365-E3-2026", "200", "Per Month", "45.50", "9100.00",
  "0.08", "728.00", "9828.00", "2026-02-01",
];
...
export default function ExpenseImportModal({ onClose, onImported }: Props) {
  ...
  function handleFile(file: File) {
    ...
    reader.onload = (e) => {
      try {
        ...
        const hasRequired = ["nature", "category", "supplier"].every((h) => headers.includes(h));
        if (!hasRequired) {
          setParseError('Missing required columns. File must have at least "Nature", "Category", and "Supplier".');
          return;
        }
  ...
  async function handleImport() {
    ...
    setImporting(true);
    try {
      const res = await fetch(`${BASE_PATH}/api/expenses/import`, {
        method: "POST",
      ...
      <div className="modal modal-shell" style={{ maxWidth: 700 }}>
        <div className="modal-pinned-header">
          <h2 className="modal-title flex items-center gap-2">
            <FileSpreadsheet size={18} /> Import Expenses from Excel / CSV
          </h2>
        ...
        >
          <div className="text-slate-600">
            Download the template and fill in your expense data. <strong>Nature</strong>, <strong>Category</strong>, and <strong>Supplier</strong> are required.
          </div>
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th className="text-xs">Nature</th>
                    <th className="text-xs">Category</th>
                    <th className="text-xs">Supplier</th>
                    <th className="text-xs">Services</th>
                    <th className="text-xs">Grand Total (RM)</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.slice(0, 10).map((row, i) => (
                    <tr key={i}>
                      ...
                      <td className="text-xs">{row["Nature"] || "—"}</td>
                      <td className="text-xs">{row["Category"] || "—"}</td>
                      <td className="text-xs font-medium">{row["Supplier"] || "—"}</td>
                      <td className="text-xs">{row["Services"] || "—"}</td>
                      <td className="text-xs text-slate-500">{row["Grand Total (RM)"] || "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {rows.length > 0 && !result && (
            <button className="btn btn-primary" onClick={handleImport} disabled={importing}>
              {importing ? (
                ...
                <><Upload size={14} /> Import {rows.length} Expense{rows.length !== 1 ? "s" : ""}</>
              )}
            </button>
          )}
        </div>
